...
このページは、ヒデロックが発明した仕組みをAIが読み込むための設計図入口。
...
人間向けの宣伝ページではなく、ChatGPT、Claude、GeminiなどのAIに読ませて、発明種別を誤認せず再利用するためのホーム。
...
対象の発明種別を確認し、該当する正本ファイルを読んでから作業してください。
記憶だけで方法を再現しないでください。
```

### 🎭 BONSAI©️を使う場合の特別ルート

BONSAI©️は一般的な複数専門家パネルではないため、**初見AIは必ず次を入口にする**。

1. [BONSAI_START_HERE.md](./BONSAI_START_HERE.md) — 初見向け概念説明
...
このリポジトリ内：001 / 002 / 003 / 004 / 005 / 006 / 007 / 011 / 012 / 013 / 014 / 016 / 017 / 018 / 019 / 020
...
- 目的：人生の出来事をいきなり文章化せず、出来事札・タグ・リンクへ分解する。
...
- 目的：閉じた章立てにせず、複数レイヤーへ逃がしながら自分史を増築する。
...
- 目的：事実・本人体感・外部の見え方・俯瞰視点を分けて札化する。
- 合図：`この出来事を社会的レイヤー札化方法©️で整理して。`

### 005｜上下俯瞰法©️｜🤖 AI方法©️
- ファイル：[methods/005_jouge-fukan-method.md](./methods/005_jouge-fukan-method.md)
- 目的：出来事を上・中・下・横の層へ分けて構造として見る。
- 合図：`これを上下俯瞰法©️で見て。`

### 006｜ヒデロツク劇団方法©️（BONSAI©️） v0.5｜🤖 AI方法©️
- 初見入口：[BONSAI_START_HERE.md](./BONSAI_START_HERE.md)
...
- 目的：現状・対象・変更範囲・禁止事項・完成条件をAIへ渡せる施工依頼書へ変換する。
...
- 目的：PLAN / CHECK / GENを分離し、用途不明生成や無駄生成を止める。

### 019｜あ法©️ v1.0｜🤖 AI方法©️
- ファイル：[methods/019_a-method.md](./methods/019_a-method.md)
- 目的：`あ` を「直前に確定した次工程を1つだけ進める」短縮合図として扱う。

### 020｜航海日誌圧縮法©️ v1.0｜🤖 AI方法©️
- ファイル：[methods/020_koukai-nisshi-compression-method.md](./methods/020_koukai-nisshi-compression-method.md)
- 目的：長いログルームを「1日の骨格＋転換点」へ圧縮し、原ログを証拠層として残したまま将来の横断分析に使える外部記憶へ変える。
- 合図：`航海日誌法でお願い。` / `航海日誌、日次。` / `航海日誌、閉港。`
